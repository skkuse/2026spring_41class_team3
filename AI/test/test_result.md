...
> - **Recall이 아닌 F1 Score를 Metric으로 설정한 이유**: `Recall >= 0.45`는 요약이 정상적임에도 원문과 생성 텍스트 간 길이 차이(단어 수 부족 등)로 인해 **30%(6개 파일)를 탈락**시킵니다. 요약은 원문의 전체 내용을 담아야 하는 것이 아닌 핵심 내용을 전달하는 것이 목적이므로, Precision과 Recall의 균형을 보는 **F1 Score**가 적합